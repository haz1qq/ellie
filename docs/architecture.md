...
`TaskService` uses short-lived SQLite connections and immediate write transactions. Its first bootstrap creates one internal `My tasks` list when storage is empty; the main UI presents a single aggregate board rather than list administration. Task text, explicit `work|personal` type, priorities, calendar dates, and optional repository identity snapshots are validated in Rust; only Work tasks may retain a repository. Completing/deleting the pinned task clears Focus transactionally. Tasks do not call GitHub, do not become Issues, and survive GitHub disconnect.
...
The `task-note` Tauri window is separate from the unchanged `mini` quota bar. It is hidden until an incomplete task is pinned, then appears always on top, taskbar-free, undecorated, and draggable. Rust restores a monitor-validated SQLite position and targets task updates only to that window. Its dedicated capability can read, complete, or unpin only the current pinned task and restore the main window; it cannot enumerate or edit other tasks, access GitHub/providers/settings/credentials, or open arbitrary URLs. Note-origin completion/unpin emits a main-window refresh event. Completing, deleting, or unpinning the task hides the note and clears Focus transactionally.
