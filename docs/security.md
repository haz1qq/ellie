...
Task IPC is main-window-only and Rust validates IDs, bounded text, enum values, strict calendar dates, repository identities, and deletion counts. Repository URLs are derived from validated `owner/name`; the WebView cannot supply arbitrary URLs. Task writes and pin clearing are transactional and run on blocking workers. Task notes, due dates, and private repository link snapshots are ordinary unencrypted SQLite data protected only by the Windows user boundary, not Credential Manager. Tasks are never exposed through the existing local API/CLI and survive GitHub disconnect.
