...
- Local to-do workspace: create/rename/delete lists; create/edit/complete/reopen/delete tasks; priorities, calendar due dates, optional repository snapshots, filters, and one pinned Focus task. Tasks remain local, persist until explicit deletion, and never synchronize to GitHub Issues.
- Local SQLite with transactional, versioned migrations (settings, providers, accounts, snapshot history, windows, token usage, notification rules/state, task lists/tasks, and minimal uncertain repository-creation attempt metadata).
