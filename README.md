...
- Optional mini floating bar: a separate always-on-top, taskbar-free, **user-resizable** window showing live provider-reported quota remaining. It omits providers without quota windows and keeps zero, missing, unavailable, and stale states distinct. Settings controls enablement, 50–100% translucent surface opacity while labels stay opaque, section opt-ins (GitHub activity and current task, default off), and a reset-position recovery; the chosen size persists and never shrinks below the enabled sections. Dragging persists a monitor-validated local position, and clicking restores the main Overview. The task section shows the pinned task, or the next open task labelled "Next task" when nothing is pinned. It reuses cached normalized data and provider update events plus the shared GitHub commit cache, so opening it does not trigger provider or GitHub network work.
